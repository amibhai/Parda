...
**Status:** Finalized for Phase 1 + Sub-Phase 2A + Sub-Phase 2B + Phase 3 (3A-3D) + Phase 4 (4A-4D) + Phase 4.5 (4.5A-4.5E), plus on-device Android verification — v0.5.1 | Last updated: 2026-08-07
...
| Real Android BLE mesh backend (Rust↔JNI) | ⚠️ Implemented; **advertising verified on real hardware**, peer exchange not | `mobile-bridge/` + `MeshBridge.kt`. Verified on a Pixel 8 via `dumpsys bluetooth_manager`: `com.parda.app` an active advertiser, `Legacy: false`, `Connectable: true`, no device name, service data carrying the rotating token. **Two-device discovery and bundle exchange still never observed; passive-scanner test never run against this backend on real radio** |
...
| Mobile mesh integration (native bridge) | ✅ Driven from the app, with the runtime permission flow | `MeshPlugin` (now `ActivityAware`) requests `BLUETOOTH_ADVERTISE`/`SCAN`/`CONNECT`; Settings toggles mesh on and off. Permission grant and advertising both confirmed on a Pixel 8 |
| Decrypted plaintext held in a native zeroize-on-release buffer rather than a Dart `String` | ✅ Implemented & tested — Sub-Phase 4.5C (narrows the window; does not eliminate it — see §3.4) | `protocol/src/plaintext_ffi.rs` (3 unit tests + a Linux `/proc/self/mem` canary test); `mobile-bridge/src/plaintext_exports.rs`; `mobile/lib/crypto/plaintext_handle.dart`. End-to-end path exercised on a Pixel 8: a received message decrypts natively and renders through a handle |
| Scrubbing the Dart-side platform-channel buffer | 🔲 **Not possible on this platform** — found on device, see §3.4 | Flutter returns an unmodifiable `Uint8List`; `fillRange` throws. Zeroing is best-effort and that copy persists until GC |
...
| User-facing fingerprint display | ✅ Implemented — Android safety-number screen | `mobile/lib/screens/safety_number_screen.dart`; 60 digits, verified on a Pixel 8 to match `trust.rs` exactly (`fingerprint_matches_the_android_implementation_known_answer`) |
| User-facing fingerprint **enforcement** | 🔲 Not implemented — see §3.8 | The Android client stores no verified state and will not warn on a later key change; the screen says so |
